 ...
 * <p>
 * 主要是读取application中数据源的配置，先读取spring.datasource.master构建默认数据源,然后在构建cluster中的数据源。
 * <p>
 * 在这里注册完数据源之后，我们需要通过@import注解把我们的数据源注册器导入到spring中 在启动类Chapter5Application.java
 * 加上如下注解@Import(DynamicDataSourceRegister.class)。
 * <p>
 * 其中我们用到了一个DynamicDataSourceContextHolder 中的静态变量来保存我们已经注册成功的数据源的key,至此我们的数据源注
 * 册就已经完成了。
 *
 * @author xuweizhi
 * @since 2019/04/25 11:01
 */
